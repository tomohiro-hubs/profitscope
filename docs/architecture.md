...
## 1. 目的
本ドキュメントは、ProfitScope の実装境界を固定し、Cloudflare Workers + D1 の本番運用を前提に、複数エージェントが並列実装しても整合性を保てるようにするための設計契約を定義する。
...
- `/lib/auth`: 認証レイヤー。パスワード照合、セッション生成・削除、Cookie 制御を担当。
...
- `app/api/auth -> auth`: ログイン・ログアウト・認証確認処理を委譲する。

### 4.1 依存方向
- 許可: `app/components -> lib/* -> types`
- 許可: `app/api -> lib/persistence -> types`
- 許可: `app/api/auth -> lib/auth -> lib/db`
...
8. ログイン時は `app/api/auth/login` が `lib/auth` を呼び出し、D1 `users` / `sessions` で認証を行う。
...
- `auth` は PBKDF2 による照合とセッション発行のみを担当し、画面表示責務を持たない。

## 8. Cloudflare Workers 運用方針
- 本番は Cloudflare Workers 上で Next.js(App Router) を実行する。
- D1 は認証情報とダッシュボード状態の単一永続化先とする。
- GitHub Pages の静的配信は API 非対応のため、本番構成から除外する。
